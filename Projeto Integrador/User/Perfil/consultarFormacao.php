...
if (isset($_GET['excluir']) && !empty($_GET['excluir'])) {
    ...
    $sqlExcluir = "DELETE FROM formacao WHERE idFormacao = ? AND idCpfCandidato = ?";
    ...
    if (mysqli_stmt_execute($stmtExcluir)) {
        ...
        echo "<script>alert('Exclusão Bem-Sucedida!'); window.location.href = './consultarFormacao.php';</script>";
        exit;
    } else {
...
$sql = "SELECT * FROM formacao WHERE idCpfCandidato = ?";
...
<body>
        ...
        <div class="content">
            ...
            <h1>Suas Formações</h1>
    <?php
    // Loop através dos resultados da consulta
    while ($row = mysqli_fetch_assoc($result)) {
        ?>
        <div class="card">
            <h2><?php echo htmlspecialchars($row['curso']); ?></h2>
            <p><strong>Instituição:</strong> <?php echo htmlspecialchars($row['instituicao']); ?></p>
            <p><strong>Situação:</strong> <?php echo htmlspecialchars($row['situacao']); ?></p>
            <a class="excluir-btn" href="./consultarFormacao.php?excluir=<?php echo $row['idFormacao']; ?>">Excluir</a>
        </div>
        <?php
    }
    ?>
